...
CREATE POLICY "Only admins can insert gallery images" ON gallery_images FOR INSERT WITH CHECK (
  auth.uid() IN (SELECT id FROM admin_users)
);
CREATE POLICY "Only admins can update gallery images" ON gallery_images FOR UPDATE USING (
  auth.uid() IN (SELECT id FROM admin_users)
);
CREATE POLICY "Only admins can delete gallery images" ON gallery_images FOR DELETE USING (
  auth.uid() IN (SELECT id FROM admin_users)
);
...
INSERT INTO articles (title, slug, excerpt, content, image_url, author) VALUES
('Selamat Datang di MIT Amanah', 'selamat-datang-mit-amanah', 'Kami dengan bangga mempersembahkan website resmi MIT Amanah', 'MIT Amanah adalah sebuah institusi pendidikan yang berkomitmen untuk memberikan pendidikan berkualitas tinggi. Dengan fasilitas modern dan tenaga pengajar profesional, kami siap membimbing generasi muda menuju masa depan yang cerah.', '/placeholder.svg?height=400&width=800', 'Admin MIT Amanah'),
('Program Pembelajaran Inovatif', 'program-pembelajaran-inovatif', 'Kami menawarkan berbagai program pembelajaran yang dirancang khusus', 'Program pembelajaran kami dirancang dengan kurikulum internasional yang disesuaikan dengan kebutuhan lokal. Setiap siswa mendapatkan perhatian personal dan dukungan akademik yang komprehensif untuk mencapai potensi maksimal mereka.', '/placeholder.svg?height=400&width=800', 'Admin MIT Amanah'),
('Ekstrakurikuler dan Pengembangan Karakter', 'ekstrakurikuler-pengembangan-karakter', 'Program ekstrakurikuler yang komprehensif untuk mengembangkan potensi siswa', 'Selain akademik, kami juga menyediakan berbagai kegiatan ekstrakurikuler mulai dari olahraga, seni, hingga pengembangan kepemimpinan. Kegiatan ini dirancang untuk mengembangkan karakter, kepercayaan diri, dan keterampilan sosial siswa.', '/placeholder.svg?height=400&width=800', 'Admin MIT Amanah');
